                              ...
                              const cyw43_ev_scan_result_t *result)
{
    ...
    *offset = 0;
    ...
    syslog("wifi_scan: ssid %-15s bssid %s", result->ssid, mac);
    net->wcn_found_networks++;
...
static i32 wifi_connect(struct net *net)
{
    ...
    strcpy((char *) scan_opts.ssid, net->ssid);
    ...
    if (!net->wcn_found_networks) {
        syslog("No network named %s found", net->ssid);
        return 1;
    }

    err = cyw43_arch_wifi_connect_timeout_ms(
        net->ssid, MICRON_CONFIG_NET_PASSWD, CYW43_AUTH_WPA2_AES_PSK, 10000);
    if (err) {
        syslog("Failed to connect to Wi-Fi network");
        return 1;
    }

    syslog("Connected to %s network", net->ssid);
    net->wcn_connected = true;
...
i32 net_init(struct net *net)
{
    ...
    net->ssid = MICRON_CONFIG_NET_SSID;
